...
function solution(schedules, timelogs, startday) {
    ...
    const isSafe = (schedule, timeLog) => {
        const hour = Math.floor(schedule / 100); // 희망 출근 시각의 시간 부분
        const min = schedule % 100 + 10; // 희망 출근 시각의 분 부분 + 10분

        // 시간 계산 후 허용 마감 시각 계산 (분이 60을 넘으면 시간으로 올림)
        const after10 =  Math.floor(hour + min / 60)* 100 + (min % 60);
    ...
    for (let i = 0; i < timelogs.length; i++) {
        ...
        let flag = true; // 해당 직원이 상품을 받을 수 있는지 여부 (true: 상품 받을 수 있음)

        // 일주일 동안의 각 날짜별 출근 기록 확인
        for (let j = 0; j < 7; j++) {
            const day = (j + startday - 1) % 7; // 실제 요일 계산 (0: 월요일 ~ 6: 일요일)
...
let timelogs = [[710, 2359, 1050, 700, 650, 631, 659], [800, 801, 805, 800, 759, 810, 809], [1105, 1001, 1002, 600, 1059, 1001, 1100]];
console.log(solution(schedules, timelogs, 5));
...
console.log(solution(schedules, timelogs, 1));